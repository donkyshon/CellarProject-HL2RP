...
ENT.Sequential			= true
ENT.MaxRange			= 10000
ENT.model				= "models/sw/avia/bombs/9m114.mdl"
ENT.TkAmmo 				= 1
ENT.Kind				= "rocket_9m114"
ENT.FaF					= true
sound.Add( {
	name = "fire",
	channel = CHAN_AUTO,
	...
	if self.FaF then
		draw.Text({
			text = (
				...
				and (IsValid(self:GetTarget()) and "ЗАХВАТ" or "РАКЕТА ГОТОВА")
				or "РАКЕТА НЕ ГОТОВА"
		...
		draw.Text({
			text = (
				self:GetNextShot() <= CurTime() and self:GetAmmo() > 0
				and (IsValid(self:GetTarget()) and "ЗАХВАТ" or "НЕТ ЦЕЛИ")
				or "РАКЕТА НЕ ГОТОВА"
