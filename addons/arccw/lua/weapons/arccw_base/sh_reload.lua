-- CELLAR ENTRANCE #1:
local function GetItemAmmo1(client, weapon)
    if !weapon.AmmoItem then return 0 end

    local character = client:GetCharacter()
    local inventory = character:GetInventory()
    local value = 0

    for k, item in ipairs(inventory:GetItemsByUniqueID(weapon.AmmoItem, true)) do
        value = value + item:GetStacks()
    end
...
function SWEP:Reload()
    ...
    if GetItemAmmo1(self:GetOwner(), self) <= 0 then return end --self:Ammo1() <= 0 then return end
...
local function GetAmmoItemForCount(client, count, weapon)
    local load = 0
    local character = client:GetCharacter()
    local inventory = character:GetInventory()

    local ammo = inventory:GetItemsByUniqueID(weapon.AmmoItem, true)
    table.sort(ammo, function(a, b) return a:GetStacks() < a:GetStacks() end)

    local items = {}
    for i, item in ipairs(ammo) do
        local value = item:GetStacks()
...
function SWEP:RestoreAmmo(count)
    ...
    if count > self:Clip1() then
        count = count - self:Clip1() 
    end

    local test1, ammo = GetAmmoItemForCount(self:GetOwner(), count, self)
    local load = math.Clamp(self:Clip1() + test1, 0, clip + chamber)

    if SERVER then
        for item, value in pairs(ammo) do
            local delta = item:GetStacks() - value

            if delta == 0 then
                item:Remove()
            else
                item:SetData("stack", item:GetStacks() - value)
            end
        end
    end
...
function SWEP:ReloadInsert(empty)
    ...
    if self:Clip1() >= total or self:GetNWBool("reqend", false) then
