...
----------------------------------------
local function GetSequenceForWeapon(player, isMoving)
	local animations = player.ixAnimTable

	if !animations then
		return "pidle_normal"
	end

	local act = isMoving and "prone_walk" or "prone_idle"
	local animation = animations[act]

	if istable(animation) then
		animation = animations[act][player:IsWepRaised() and 2 or 1]
	end
...
local GetMainActivityAnimation = {
	...
	[PRONE_INPRONE] = function(ply, velocity)
		return GetSequenceForWeapon(ply, velocity:LengthSqr() >= 225)
	end,
