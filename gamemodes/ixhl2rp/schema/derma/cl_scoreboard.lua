...
function PANEL:Update()
	...
	local isDonator = client:IsDonator()
	if self.donator != isDonator then
		self.donator = isDonator

		self.name:SetTextColor(self.donator and donatorClr or color_white)
		self.description:SetTextColor(self.donator and donatorClr or color_white)
	end
end
...
function PANEL:Init()
	...
	for i, data in SortedPairsByMemberValue(available, 1, true) do
		...
		panel.Paint = function(me, w, h)
			...
			draw.SimpleText(data[2] and L(i):utf8upper(), 'cellar.main.btn', w - 6, 14, Color(56, 207, 248), TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
			draw.SimpleText(data[2] and L(i):utf8upper(), 'cellar.main.btn.blur', w - 6, 14, Color(56, 61, 248, 225), TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
